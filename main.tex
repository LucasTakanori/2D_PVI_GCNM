...
\end{equation}
Here, $\operatorname{NRMSE}_{\mathrm{val}}$ is the normalized validation root mean squared error, $\operatorname{RMS}_{\mathrm{background,val}}$ is the validation root mean squared error over target-background elements, and $\operatorname{Dice}_{\mathrm{support,val}}$ compares the true nonzero support with an equal-cardinality set of elements having the largest absolute predictions.
...
\color{red}
NEEDED OR WE CAN REMOVE THE TABLE?
The complete ring-specific dataset inventory is summarized in Table~\ref{tab:gcnm-synthetic-inventory}.

\begin{table}[htbp]
    
    \centering
    \caption{Synthetic datasets generated for ring-specific GCNM training.
    Every beat contains 50 ordered samples.}
    \label{tab:gcnm-synthetic-inventory}
    \small
    \begin{tabular}{lrrrr}
        \hline
        Partition & Anatomies & Beats & Samples per ring & All-ring samples \\
        \hline
        Training & 160 & 800 & 40,000 & 600,000 \\
        Validation & 20 & 100 & 5,000 & 75,000 \\
        Test & 20 & 100 & 5,000 & 75,000 \\
        \hline
        Total & 200 & 1,000 & 50,000 & 750,000 \\
        \hline
    \end{tabular}
\end{table}
...
Before training all ring-specific models, we selected the architecture and loss coefficients in a controlled US120 pilot shown in Supplementary Fig.~24b.
This pilot compared direct and residual outputs, the inclusion of element coordinates, target-magnitude weighting, and background weighting.
The selected formulation used the direct output with coordinates, $\alpha=1$, and $\beta=0.25$, while $\lambda=5\times10^{-4}$ and the 64-channel hidden width were held fixed rather than exhaustively optimized.

We compared the conventional Newton reconstruction and both GCNM stages with the known conductivity target on a standardized exact nonlinear visualization set containing three complete beats from one test anatomy for each ring.
Across the 15 ring-specific sequences, the median NRMSE was $4.72$ for Newton, $1.12$ for the first GCNM stage, and $1.12$ for the second stage.
The corresponding median spatial correlations were $-0.013$, $-0.090$, and $-0.075$.
Visual inspection of Supplementary Fig.~24a showed that the GCNM stages suppressed the large diffuse background patterns produced by Newton and recovered two spatially distinct regions corresponding to the synthetic arterial targets.
Relative to the first stage, the second stage provided a small aggregate improvement, with the mean NRMSE decreasing from $1.20$ to $1.19$ and the mean spatial correlation increasing from $-0.060$ to $-0.053$ across the 15 ring-specific sequences.
...
The lower GCNM NRMSE therefore reflected suppression of the large Newton reconstruction error together with recovery of a more localized two-vessel structure.

After reconstruction, the elementwise Newton and GCNM outputs were rasterized on the $40\times40$ grid defined in Supplementary Discussion~6.1.
Because direct conductivity ground truth was unavailable for the experimental recordings, their temporal behavior was compared qualitatively in the participant atlas shown in Supplementary Fig.~24c.
...
Ten approximately equally spaced samples with indices $0$, $5$, $11$, $16$, $22$, $27$, $33$, $38$, $44$, and $49$ will span the beat from its first to its final sample.
For every participant, the Newton reconstructions will be displayed in the first row and the corresponding second-stage GCNM reconstructions in the second row.
Spatial orientation, image extent, sample index, and conductivity units will be identical between the two rows.
A common participant-specific color scale will be used across both methods and all ten samples so that changes in amplitude and spatial concentration remain directly comparable.
...
\begin{figure}[p]
    ...
    \textbf{(a)}\par
    \IfFileExists{figures/gcnm_synthetic_mesh_anatomy_comparison.pdf}{
        \includegraphics[
            width=\textwidth,
            height=0.20\textheight,
            keepaspectratio
        ]{figures/gcnm_synthetic_mesh_anatomy_comparison.pdf}
    }{
        \fbox{
            \parbox[c][0.14\textheight][c]{0.92\textwidth}{
                \centering
                Synthetic examples from different ring meshes and virtual anatomies,
                followed by ground truth, Newton, and second-stage GCNM reconstructions.
            }
        }
    }
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{figures/gcnm_parameter_selection.pdf}

    \vspace{0.6em}
    \textbf{(c)}\par
    \IfFileExists{figures/gcnm_subject_beat_atlas_preview.pdf}{
        \includegraphics[
            width=\textwidth,
            height=0.20\textheight,
            keepaspectratio
        ]{figures/gcnm_subject_beat_atlas_preview.pdf}
    }{
        \fbox{
            \parbox[c][0.14\textheight][c]{0.92\textwidth}{
                \centering
                Participant beat-atlas placeholder with ten columns for samples
                $0$, $5$, $11$, $16$, $22$, $27$, $33$, $38$, $44$, and $49$.\par
                Top row: Newton reconstruction. Bottom row: GCNM stage 2.
            }
        }
    }

    \caption{\textbf{Supplementary Fig.~24. Synthetic data generation, GCNM selection, and reconstruction comparison.}
    \textbf{a,} Representative synthetic data from different ring meshes and virtual anatomies, together with the corresponding conductivity ground truth, conventional Newton reconstruction, and second-stage GCNM reconstruction.
    \textbf{b,} Controlled US120 architecture and loss ablation used to select the direct coordinate formulation with $\alpha=1$ and $\beta=0.25$ before applying the same formulation to all 15 ring meshes.
    \textbf{c,} Participant-level temporal comparison of conventional Newton and second-stage GCNM reconstructions at ten samples spanning one complete beat.}
    \label{fig:supp24}
\end{figure}
