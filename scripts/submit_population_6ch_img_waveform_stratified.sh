...
for target in \
  ...
  if [[ -e "${target}" ]]; then
    echo "refusing to overwrite existing image-to-waveform experiment: ${target}" >&2
    exit 2
  fi
done
...
echo "CRT and Samba six-channel image-to-waveform training (two GPUs concurrently): ${train_job}"
