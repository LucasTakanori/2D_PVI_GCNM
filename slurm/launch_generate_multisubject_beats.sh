#!/bin/bash
#SBATCH --job-name=gcnm-beat-data
#SBATCH --output=logs/gcnm-beat-data_%j.out
#SBATCH --error=logs/gcnm-beat-data_%j.err
...
python -u -m gcnm_pvi.generate_multisubject_beat_dataset \
  ...
  --out-dir "${DATASET_DIR:?set DATASET_DIR}" \
  --train-beats "${TRAIN_BEATS:-1000}" \
  --validation-beats "${VALIDATION_BEATS:-200}" \
  --test-beats "${TEST_BEATS:-32}" \
  --train-phases-per-beat "${TRAIN_PHASES_PER_BEAT:-4}" \
  --validation-phases-per-beat "${VALIDATION_PHASES_PER_BEAT:-4}" \
  --train-nonlinear-fraction "${TRAIN_NONLINEAR_FRACTION:-1.0}" \
  --validation-nonlinear-fraction "${VALIDATION_NONLINEAR_FRACTION:-1.0}" \
  --seed "${SEED:-20260716}"
