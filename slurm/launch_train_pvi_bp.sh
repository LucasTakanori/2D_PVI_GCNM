#!/bin/bash
#SBATCH --job-name=gcnm-pvi-bp
#SBATCH --output=logs/gcnm-pvi-bp_%A_%a.out
#SBATCH --error=logs/gcnm-pvi-bp_%A_%a.err
...
echo "task=${TASK_ID} subject=${SUBJECT} target=${TARGET} mask=${MASK_KEY} channels=${CHANNEL_MODE}"
python -u -m gcnm_pvi.train_pvi_bp \
  --pvi-ml-root "${PVI_ML_ROOT:?set PVI_ML_ROOT}" \
  --parquet-root "${PARQUET_ROOT:?set PARQUET_ROOT}" \
