#!/bin/bash
# Optimized US120 coordinate HP/LP pilot export.
# Thirty-two persistent row-range workers share two GPUs and reuse the exact
# cached/vectorized PVI FEM implementation.
#SBATCH --job-name=us120-coordinate-fast
#SBATCH --output=logs/us120-coordinate-fast_%j.out
#SBATCH --error=logs/us120-coordinate-fast_%j.err
...
OUTPUT_ROOT="${REPO_ROOT}/gcnm_parquet/us120_pilot_coordinate_hp_lp_v1"
...
python -u -m gcnm_pvi.export_pvi_parquet \
  ...
  --family coordinate \
  --checkpoint-root "${REPO_ROOT}/models/hp_lp_us120_v1/coordinate" \
