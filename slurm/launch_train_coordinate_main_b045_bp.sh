#!/bin/bash
# One native pvi_ml CRT experiment per task; four tasks fill four GPUs.
#SBATCH --job-name=coord91-bp
#SBATCH --output=logs/coord91-bp_%A_%a.out
#SBATCH --error=logs/coord91-bp_%A_%a.err
...
if [[ "${TASK_ID:-}" != "${SLURM_ARRAY_TASK_ID}" || "${ARCHITECTURE}" != "crt" || "${MASK_KEY}" != "mask05" ]]; then
  echo "invalid task-manifest row ${SLURM_ARRAY_TASK_ID}" >&2
  exit 2
fi
...
common=(
  ...
  --subject "${SUBJECT}" --family "${FAMILY}" --architecture crt
