#!/bin/bash
# Subject010 CRT waveform and fiducial experiments using Newton 3ch + coordinate 3ch.
#SBATCH --job-name=s010-newton-coord-6ch
#SBATCH --output=logs/s010-newton-coord-6ch_%A_%a.out
#SBATCH --error=logs/s010-newton-coord-6ch_%A_%a.err
...
COORDINATE_ROOT="${COORDINATE_ROOT:-${REPO_ROOT}/gcnm_parquet/subject010_coordinate_direct_s1_s2_ds2_v1}"
...
ARTIFACT_ROOT="${ARTIFACT_ROOT:-${REPO_ROOT}/artifacts/subject010_newton_coordinate_6ch_bp_v1}"
...
echo "[$(date --iso-8601=seconds)] subject010 CRT ${OUTPUT_MODE} Newton3+coordinate3"
...
echo "[$(date --iso-8601=seconds)] subject010 CRT ${OUTPUT_MODE} Newton3+coordinate3 complete"
