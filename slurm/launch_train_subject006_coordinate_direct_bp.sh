#!/bin/bash
# Two subject006 CRT experiments: waveform and fiducials.
#SBATCH --job-name=coord-s006-bp
#SBATCH --output=logs/coord-s006-bp_%A_%a.out
#SBATCH --error=logs/coord-s006-bp_%A_%a.err
...
ARTIFACT_ROOT="${ARTIFACT_ROOT:-${REPO_ROOT}/artifacts/subject006_coordinate_direct_s1_s2_ds2_bp_v1}"
...
echo "[$(date --iso-8601=seconds)] subject006 CRT ${OUTPUT_MODE} [S1,S2,dS2/dt]"
python -u -m gcnm_pvi.train_pvi_bp \
  ...
  --subject subject006 --family coordinate --architecture crt \
  --output-mode "${OUTPUT_MODE}" --channel-mode 3ch \
  --artifact-root "${ARTIFACT_ROOT}" --max-epochs 5000 --num-workers 8
echo "[$(date --iso-8601=seconds)] subject006 CRT ${OUTPUT_MODE} complete"
